...
<HTML>
<HEAD>
<TITLE>Java applet! UPload to Paint</TITLE>
<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=gb2312">
<link rel="stylesheet" href="CSS.asp" type="text/css">
<style type="text/css">
<!--
.unnamed1 {  font-family: "Verdana", "Arial", "Helvetica", "sans-serif"; font-size: 14px; text-decoration: none}
-->
</style>
</HEAD>
...
<div align="center">
  ...
  <span class="unnamed1"><b><font color="#006699">保存作品后需要刷新才能看到！</font></b></span> 
...
//-->
  ...
  <%if Request.QueryString("w") and Request.QueryString("h") and Request.QueryString("paint")<>""then 
dim w,h,pname,paint
paint=Request.QueryString("paint")
w=Request.QueryString("w")
pic=Request.Cookies("Uploadname")
h=Request.QueryString("h")
pname=Request.QueryString("name")
pop=Request.QueryString("pop")
 %>
  <%If paint= "painter" then %>
  <%pch= ""%>
</div>
...
<param name="url_exit" value="<%=user_name%>/write.asp?picname=<%=pic%>&uploadpaint=Shi-painter"> 
...
<param name="image_canvas" value="<%=user_name%>/picdata/<%=pic%>">
 </applet></td></tr> </table></td><td width="146" align="left" valign="top" nowrap bgcolor=#cccccc> 
...
<param name="readpicpath" value="picdata/">
 <param name="readpic" value="<%=pic%>">
  <param name="url" value="write.asp?picname=<%=pic%>&uploadpaint=OekakiBBS"> 
<param name="cgi" value="saveOE.asp?Action=save&pic=<%=pic%>&pch=temp.Oeb">
 <param name="popup" value="0"> 
<param name="tooltype" value="full">
 <param name="anime" value="1"> 
 <param name="animesimple" value="0"> 
<param name="tooljpgpng" value="0">
 <param name="toolpng" value="0"> 
...
//-->
        ...
        <applet code="c.ShiPainter.class" codebase="../" name="paintbbs" width="720" height="550" archive="spainter.jar,res/pro.zip" MAYSCRIPT align=top>
          ...
          <param name="color_iconselect" value="#cccccc">
          <param name="url_save" value="<%=user_name%>/saveSP.asp?pic=<%=pic%>&pch=<%=pch%>&PType=jpg">
          <param name="url_exit" value="<%=user_name%>/write.asp?picname=<%=pic%>&uploadpaint=Shi-painterPro">
