...
book.Source = "SELECT * FROM book WHERE good_Topic=1 AND use_tool <> '" + Replace(book__MMColParam, "'", "''") + "' ORDER BY book_ID DESC"
book.CursorType = 0
                ...
                <td bgcolor="#CCCCCC"> 
                  <div align="center"> 
                    ...
                    <A HREF="showToppic.asp?<%= MM_keepNone & MM_joinChar(MM_keepNone) & "book_ID=" & book.Fields.Item("book_ID").Value %>" title="实际尺寸:<%=book("pic_W")&"×"&book("pic_H")%>, 大小:<%=book("filesize")&"KB"%>,&nbsp;Min: 
                          <%if h1>=image_width_out then Response.Write"height="&image_width_out&"" else Response.Write"width="&image_w&""   end if %>" target="_blank"><img src='picdata/<%=book("picname")%>' <%if h1=<picmode_width then Response.Write"width="&image_w&"" else Response.Write"height="&picmode_width&""  end if %> border="0"></A></div>
                </td>
              </tr>
            </table>
          &nbsp;&nbsp; 
                  <%if Session("admin_name")=user_name or  Session("admin_pass")=user_password then %>
            <a href="file_del_poss_admin.asp?<%= MM_keepNone & MM_joinChar(MM_keepNone) & "ID=" & book.Fields.Item("book_ID").Value %>&picname=<%=book.Fields.Item("picname").Value%>&pchname=<%=book.Fields.Item("pchname").Value%>&url=good_Topic.asp" title="删除图片和关联帖子">Delete</a>&nbsp;&nbsp;&nbsp; 
            <% end if%>
            <%
if session("fuadmin")<>"" then 
