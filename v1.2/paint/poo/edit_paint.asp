...
<!--#include file="user.asp" --><!--#include file="color.asp" -->
<%
...
<%if Request.Cookies("guest_pass")=book.Fields.Item("pass").Value or Request.Cookies("guest_pass")=user_password then 
dim w,h,piname,pchname,paint,edit,pname
paint=Request.QueryString("tool")
w=book.Fields.Item("pic_W").Value
h=book.Fields.Item("pic_H").Value
picname=book.Fields.Item("picname").Value
pchname=book.Fields.Item("pchname").Value
edit=Request.QueryString("edit")
pname=book.Fields.Item("name").Value
ID=book.Fields.Item("Book_ID").Value
 %>
<%If paint= "Shi-painter" then %>
<br>
<br>
<br>
<form name="form1" method="get" action="edit_paint.asp">
  <table width="325" border="0" cellspacing="1" cellpadding="1" bgcolor="<%=td_bor%>" align="center">
    <tr> 
    ...
    <tr> 
      <td bgcolor="<%=bg_color%>"> 
        <table width="100%" border="0" cellspacing="1" cellpadding="1" align="center">
          ...
          <tr> 
            <td>该图象拥有两种打开方式，请您选择：</td>
          </tr>
          <tr> 
            <td> 
              <input type="radio" name="tool" value="Shi-painter1" checked>
              <b> Shi-painter easy</b> </td>
          </tr>
          <tr> 
            <td> 
              <input type="radio" name="tool" value="painterpro">
              <b> Shi-painter Pro 
...
<%end if %>
<%If paint= "Shi-painter1" then %>
<%pic= "Shi_painter"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&".jpg"
		pch= "Shi_painter"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&".spch"%>
<table width="750" border="0" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#ffffff" bordercolordark="#777777">
  <tr> 
    <td align=center width="655" height="1"> 
      <script language="JavaScript">
 var h;
 var w;
 var l;
...
//-->
      ...
      <!-- 	<INPUT TYPE="button" VALUE="变比例" onclick="scale()" class="bty" style="width:60px;">
 ...
 -->
  ...
  <tr> 
    <td align="right" valign=top bgcolor=#cccccc height="410" width="655"> 
      <table width="100%" height="100%">
        <tr> 
          <td align="right" valign=top> 
            <!-- 		<table  width="100%" border="0" align="right" cellpadding="0" cellspacing="0" id="AutoNumber1">
              <tr> 
                <td align="right" valign=top> -->
            <applet code="c.ShiPainter.class" name="PaintBBS" width="100%" height="550" archive="spainter.jar,res/normal.zip" MAYSCRIPT align=right CODEBASE="../">
              ...
              <param name="url_save" value="<%=user_name%>/saveSP.asp?<%if edit="saveas" then %>pic=<%=pic%>&pch=<%=pch%><%end if %><%if edit="save" then %>pic=<%=picname%>&pch=<%=pchname%><%end if %>&PType=jpg">
              <param name="url_exit" value="<%if edit="save" then %><%=user_name%>/write_save.asp<%end if %><%if edit="saveas" then %><%=user_name%>/getoekaki.asp?tool=spainter&W=<%=w%>&H=<%=h%>&pic=<%=pic%>&pch=<%=pch%>&pname=<%=pname%>&timeStart=<%=timer()%><%end if %>">
              <param name="image_jpeg" value="true">
              ...
              <param name="compress_level" value="9">

              <param name="pch_file" value="<%=user_name%>/picdata/<%=pchname%>">
			 
            </applet></td>
        </tr>
      </table>
    </td>
    <td width="146" align="left" valign="top" nowrap bgcolor=#cccccc> 
      <table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" >
        <form name="Palette" style="margin:0px">
          <tr> 
            <td valign="top"> 
              <input type="button" value="加图层" onClick="addLayer()" class="bty" style="width:60px;" name="button">
              <input type="button" value="描边框" onClick="drawFrame(1,0x000000)" class="bty" style="width:60px;" name="button2">
              <br>
              <select name="select" size="13" style="width:70" onChange="setPalette();" class="tx">
                <option>初始系</option>
                <option>肤色系</option>
                <option>红色系</option>
                <option>黄橙系</option>
              ...
              <br>
              <input type="button" value="暂存色盘" onClick="PaletteSave()" class="bty" style="width:87px;margin-bottom:1px;" name="button2">
              <br>
              <input type="button" value="新 增" onClick="PaletteNew()" class="bty" name="button2">
              <input type="button" value="更 改" onClick="PaletteRenew()" class="bty" name="button2">
              <br>
              <input type="button" value="删 除" onClick="PaletteDel()" class="bty" name="button2">
              <input type="button" value="加 亮" onClick="P_Effect(10)" class="bty" name="button2">
              <br>
              <input type="button" value="反 转" onClick="P_Effect(255)" class="bty" name="button2">
              <input type="button" value="减 暗" onClick="P_Effect(-10)" class="bty" name="button2">
              <br>
              <img src="" height="2px"><br>
              <select name="m_m"  class="tx" style="width:58px;">
              ...
              <br>
              <nobr> 
              <input name="m_g" type="button" value="取 得" onClick="PaletteMatrixGet()" class="bty" style="width:40px;">
              <input name="m_s" type="button" value="使 用" onClick="PalleteMatrixSet()" class="bty" style="width:40px;">
              </nobr><br>
              <textarea rows="3" name="setr" cols="11" onMouseOver="this.select()" class="tx"></textarea>
            </td>
          </tr>
        </form>
        <form name="grad" style="margin:0px;">
          <tr> 
            <td height="100%" valign="top"><nobr> 
              ...
              <input type="text" name="pst" size="5" value="" onChange="Chenge_()" class="tx">
              </nobr><br>
              <nobr> 
              <select name="p_ed" onChange="GetPalette()"  class="tx">
                <option>1</option>
                <option>2</option>
                <option>3</option>
                <option>4</option>
              ...
              <input type="text" name="ped" size="5" value="" onChange="Chenge_()" class="tx">
              </nobr><br>
              <nobr> 
              <input name="ccccccc" type="button" value="---D" onClick="ChengeGrad()" class="bty" style="width:40px;">
        ...
        <form name="toForum" method="POST" style="visibility:hidden;margin:0px;">
          &nbsp; 
        </form>
      </table>
    </td>
...
<p> 
  <%end if%>
  <%If paint= "oekakibbs" or paint="OekakiBBS" then %>
<div align="center"><br>
  <applet name="oekakibbs" code="a.p.class" archive="oekakibbs.jar" width="<%if w>=450 then %>90%<%end if %><%if w<450 then %>750<%end if %>" height="<%if h>=450 then %>98%<%end if %><%if h<450 then %>550<%end if %>" mayscript CODEBASE="../">
    ...
    <param name="readpic" value="<%=picname%>">
    <param name="url" value="<%if edit="save" then %>write_save.asp<%end if %><%if edit="saveas" then %>getoekaki.asp?tool=oekakibbs&W=<%=w%>&H=<%=h%>&pname=<%=pname%>&timeStart=<%=timer()%><%end if %>">
    <param name="cgi" value="saveOE.asp?Action=save&<%if edit="save" then %>pic=<%=picname%>&pch=<%=pchname%><%end if %>">
    <param name="popup" value="0">
...
//-->
  ...
  <br>
<table width="52%" border="0" cellspacing="1" cellpadding="1" bgcolor="#999999" align="center">
  <tr> 
    <td bgcolor="#999999" bordercolor="#333333"> 
...
//-->
        ...
        <tr> 
          <td> 
            <table width="598" border="0" bgcolor="#999999" align="center">
              <tr> 
                <td width="400" bgcolor="#999999"> 
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td colspan="2"> 
                        <%pic= "paintbbs_"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&".png"
		pch= "paintbbs_"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&".pch"%>
		...
		height="<%=h1%>" CODEBASE="../">
                          ...
                          <param name="url_exit" value="<%if edit="save" then %><%=user_name%>/write_save.asp<%end if %><%if edit="saveas" then %><%=user_name%>/getoekaki.asp?tool=paintbbs&W=<%=w%>&H=<%=h%>&pic=<%=pic%>&pch=<%=pch%>&pname=<%=pname%>&timeStart=<%=timer()%><%end if %>">
                          <param name="url_save" value="<%=user_name%>/savepb.asp?action=save&<%if edit="saveas" then %>pic=<%=pic%>&pch=<%=pch%><%end if %><%if edit="save" then %>pic=<%=picname%>&pch=<%=pchname%><%end if %>">
                ...
                <td width="188"> 
                  <table border="0" cellpadding="0" cellspacing="0" width="182">
                    <td align="CENTER" valign="top" height="40" width="184"> <form name="Palette"> 
                      <table cellpadding="0" cellspacing="0" width=163 height="422">
                        <tr> 
                          <td height="428"> 
                            <table cellpadding="2" cellspacing="0" width=91% style="border-collapse: collapse" bgcolor="#E6E6E6" border="1">
                              <tr> 
                                <td> <font size=2> 
                                  <input type="button" value="暂存色盘" onClick="PaletteSave()" name="button">
                                  <br>
                                  <select name="select" size="13" style="width:100" onChange="setPalette()">
                                    <option>暂存色盘区</option>
                                    <option>皮肤系</option>
                                    <option>红系</option>
                                    <option>黄、橙系</option>
                                  ...
                                  <table border="0">
                                    <tr> 
                                      <td><font size=2> 
                                        <input type="button" value="新增" onClick="PaletteNew()" name="button23">
                                        </font></td>
                                      <td><font size=2> 
                                        <input type="button" value="更改" onClick="PaletteRenew()" name="button25">
                                        </font></td>
                                      <td><font size=2> 
                                        <input type="button" value="删除" onClick="PaletteDel()" name="button24">
                                        </font></td>
                                    </tr>
                                    <tr> 
                                      <td><font size=2> 
                                        <input type="button" value="加亮" onClick="P_Effect(10)" name="button26">
                                        </font></td>
                                      <td><font size=2> 
                                        <input type="button" value="减暗" onClick="P_Effect(-10)" name="button27">
                                        </font></td>
                                      <td><font size=2> 
                                        <input type="button" value="反转" onClick="P_Effect(255)" name="button2">
                                        </font></td>
                                    </tr>
                                  </table>
                                  <font size=2> <font color="#0055BB" size="2">MATRIX</font> 
                                  <select name="m_m">
                                    <option value=0>全部</option>
                                    <option value=1>现在</option>
                                    <option value=2>追加</option>
                                  ...
                                  <input name="m_s" type="button" value="使用" onClick="PalleteMatrixSet()">
                                  <br>
                                  <textarea rows="1" name="setr" cols="13" onMouseOver="this.select()"></textarea>
                                  </font> </td></FORM>
                              <table cellpadding="3" cellspacing="1" width=155 style="border-collapse: collapse" align="center">
                                <tr> 
                                  <form name="grad">
                                    <td> 
                                      <input type="button" value=" OK " onClick="ChengeGrad()" name="button22">
                                      ...
                                      <br>
                                      <input type="text" name="ped" size="8" value="" onChange="Chenge_()">
                                      <input type="text" name="pst" size="8" value="" onChange="Chenge_()">
                                    </td>
                                  </form>
                                </tr>
                              </table>
  ...
  <tr> 
    <td> 
      <div align="center"> 
        ...
        <BR>
        <applet code="c.ShiPainter.class" codebase="../" name="PaintBBS" width="750" height="550" archive="spainter.jar,res/pro.zip" MAYSCRIPT align=top>
          <!-- <param name="popup" value="0"> -->
          <param name="undo" value="50">
          <param name="undo_in_mg" value="12">
          <param name="image_width" value="<%=w%>">
